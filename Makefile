...
CPPFLAGS = -I$(top_srcdir)/liblinear -I/usr/include/lua -I$(top_srcdir)/libdnet-stripped/include  -I$(top_srcdir)/libpcap -I$(top_srcdir)/nbase -I$(top_srcdir)/nsock/include $(DEFS)
export CFLAGS = $(CXXFLAGS)
# CFLAGS = $(DEFS) $(INCLS)
STATIC =
LDFLAGS = -Wl,-E  -Lnbase -Lnsock/src/ $(DBGFLAGS) $(STATIC)
LIBS =  -lnsock -lnbase -lpcre $(LIBPCAPDIR)/libpcap.a $(OPENSSL_LIBS) libnetutil/libnetutil.a $(top_srcdir)/libdnet-stripped/src/.libs/libdnet.a -llua $(top_srcdir)/liblinear/liblinear.a -lsvn_client-1 -ldl 
OPENSSL_LIBS = -lssl -lcrypto
# LIBS =  -lefence -lsvn_client-1 -ldl 
# LIBS =  -lrmalloc -lsvn_client-1 -ldl 
...
all: $(TARGET) $(BUILDZENMAP) $(BUILDNDIFF) $(BUILDNPING) $(BUILDNCAT) build-nmap-update

$(TARGET): build-netutil build-liblinear build-pcap  build-nsock build-nbase build-dnet  \
	$(OBJS) main.o
	@echo Compiling nmap
	rm -f $@
...
clean:  clean-liblinear clean-pcap  clean-dnet \
clean-nsock clean-nbase clean-netutil clean-nping clean-zenmap \
clean-ncat clean-nmap-update clean-ndiff clean-tests
...
distclean:  distclean-liblinear distclean-pcap \
 distclean-dnet distclean-nping distclean-zenmap \
distclean-ncat distclean-nmap-update distclean-netutil \
...
install: install-nmap $(INSTALLNSE) $(INSTALLZENMAP) install-ncat install-nmap-update $(INSTALLNDIFF) $(INSTALLNPING)
	@echo "NMAP SUCCESSFULLY INSTALLED"

uninstall: uninstall-nmap $(UNINSTALLZENMAP) uninstall-ncat uninstall-nmap-update $(UNINSTALLNDIFF) $(UNINSTALLNPING)
